// OpenWare Universal Security Content Script
...
(async function() {
  ...
  let currentSiteAnalysis: any = null;
  ...
  async function analyzeCurrentSite() {
    ...
    try {
      ...
      console.error('OpenWare: Analysis failed', error);
    }
  ...
  function createFloatingBadge() {
    ...
    badge.id = 'openware-floating-badge';
    badge.style.cssText = `
      position: fixed;
      bottom: 20px;
      right: 20px;
  ...
  function updateFloatingBadge(analysis) {
    ...
    floatingBadge.title = `OpenWare: ${threatLevel} Risk`;

    // Pulse animation for dangerous sites
    if (threatLevel === 'CRITICAL' || threatLevel === 'HIGH') {
      floatingBadge.style.animation = 'openware-pulse 2s infinite';
    } else {
  ...
  function createWarningOverlay(analysis) {
    ...
    const existing = document.getElementById('openware-warning-overlay');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.id = 'openware-warning-overlay';
    ...
    warningBox.innerHTML = `
      ...
      <div style="display: flex; gap: 12px;">
        <button id="openware-go-back" style="
          flex: 1;
          padding: 14px 24px;
          background: #dc2626;
          color: #fff;
        ...
        <button id="openware-proceed" style="
          flex: 1;
          padding: 14px 24px;
      ...
      <div style="text-align: center; margin-top: 16px;">
        <button id="openware-whitelist" style="
          background: none;
          border: none;
          color: #666;
          font-size: 12px;
    ...
    // Event listeners
    document.getElementById('openware-go-back').addEventListener('click', () => {
      window.history.back();
    });

    document.getElementById('openware-proceed').addEventListener('click', () => {
      overlay.remove();
    });

    document.getElementById('openware-whitelist').addEventListener('click', async () => {
  ...
  function showSiteAnalysisPopup() {
    ...
    popup.id = 'openware-analysis-popup';
    popup.style.cssText = `
      position: fixed;
      top: 50%;
      left: 50%;
    ...
    popup.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        ...
        <button id="openware-close-popup" style="
          background: none;
          border: none;
          color: #999;
          font-size: 24px;
    ...
    // Close button
    document.getElementById('openware-close-popup').addEventListener('click', () => {
      popup.remove();
    });
  ...
  function scanForAddresses() {
    try {
      console.log('[OpenWare] Starting address scan on:', window.location.href);

      if (!document.body) {
        console.log('[OpenWare] Document body not ready, skipping scan');
        return;
      }
        ...
        NodeFilter.SHOW_TEXT,
        {
          acceptNode: function(node) {
            ...
            if (tagName === 'SCRIPT' ||
                tagName === 'STYLE' ||
                tagName === 'NOSCRIPT' ||
                node.parentElement.classList.contains('openware-address-wrapper') ||
                node.parentElement.classList.contains('openware-price-overlay')) {
      ...
      console.log('[OpenWare] Found', textNodes.length, 'text nodes to scan');
      ...
      textNodes.forEach(textNode => {
        try {
          ...
          if (patterns.ethereum.test(text)) {
            hasMatch = true;
            chain = 'ethereum';
            console.log('[OpenWare] Found Ethereum address in:', text.substring(0, 50));
          }
          // Check for Hedera addresses
          else if (patterns.hedera.test(text)) {
            hasMatch = true;
            chain = 'hedera';
            console.log('[OpenWare] Found Hedera address in:', text.substring(0, 50));
          }
          // Check for Solana addresses (be more careful with false positives)
            ...
            if (solanaMatches && solanaMatches.some(match => match.length >= 32 && match.length <= 44)) {
              hasMatch = true;
              chain = 'solana';
              console.log('[OpenWare] Found Solana address in:', text.substring(0, 50));
            }
          }
          ...
          console.error('[OpenWare] Error processing text node:', error);
        }
      });

      console.log('[OpenWare] Scan complete. Found and highlighted', addressesFound, 'addresses');
      ...
      if (chrome.runtime && chrome.runtime.sendMessage) {
        chrome.runtime.sendMessage({
          ...
          console.log('[OpenWare] Could not send scan results:', err.message);
        });
      }
    } catch (error) {
      console.error('[OpenWare] Error in scanForAddresses:', error);
    }
  }
  ...
  function wrapAddressWithOverlay(textNode, chain) {
    try {
      if (!textNode || !textNode.parentNode || !textNode.textContent) {
        console.log('[OpenWare] Invalid text node, skipping');
        return;
      }
      ...
      if (!matches || matches.length === 0) {
        console.log('[OpenWare] No matches found for pattern');
        return;
      }

      console.log('[OpenWare] Wrapping', matches.length, 'addresses with overlay');

      // Create a wrapper with highlighted addresses
      const wrapper = document.createElement('span');
      wrapper.className = 'openware-address-wrapper';
      let lastIndex = 0;
      let html = '';
      ...
      matches.forEach(address => {
        ...
        html += `<span class="openware-address" data-address="${escapeHtml(address)}" data-chain="${chain}" style="
      ...
      if (textNode.parentNode) {
        ...
        console.log('[OpenWare] Successfully wrapped addresses');

        // Add hover listeners to all address spans
        wrapper.querySelectorAll('.openware-address').forEach(addressSpan => {
          let priceOverlay: any = null;
          ...
          addressSpan.addEventListener('mouseenter', () => {
            (addressSpan as HTMLElement).style.background = 'rgba(34, 197, 94, 0.3)';
          });

          addressSpan.addEventListener('mouseleave', () => {
            (addressSpan as HTMLElement).style.background = 'rgba(34, 197, 94, 0.15)';
          });
        });
      }
    } catch (error) {
      console.error('[OpenWare] Error wrapping addresses:', error);
    }
  }
  ...
  async function showPriceOverlay(addressElement) {
    ...
    // Create overlay
    const overlay = document.createElement('div');
    overlay.className = 'openware-price-overlay';
    overlay.style.cssText = `
      position: absolute;
      background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%);
      border: 2px solid #22c55e;
  ...
  // Initialize
  async function init() {
    console.log('[OpenWare] Initializing on:', window.location.href);
    ...
    if (CONFIG.enableAutoScan) {
      ...
      setTimeout(async () => {
        console.log('[OpenWare] Running scans...');
        injectStyles();
        createFloatingBadge();
      ...
      console.log('[OpenWare] Auto-scan disabled');
    }
  }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.action === 'rescanPage') {
      console.log('[OpenWare] Rescanning page...');
      analyzeCurrentSite();
      scanForAddresses();
  ...
  // Start
  console.log('[OpenWare] Content script loaded, readyState:', document.readyState);

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      console.log('[OpenWare] DOMContentLoaded event fired');
      init();
    });
  } else {
    console.log('[OpenWare] Document already loaded, initializing now');
    init();
  }

  // Also scan on window load for dynamic content
  window.addEventListener('load', () => {
    console.log('[OpenWare] Window load event - rescanning for addresses');
    setTimeout(() => {
      scanForAddresses();
    }, 2000);
  });
